...
interface Props {
  ...
  currentMax: string;
  currentSort: string;
  /** "sidebar" = chỉ render nội dung (page tự wrap). "mobile" = chỉ button+drawer. "auto" = tự handle cả 2 (default). */
  mode?: "auto" | "sidebar" | "mobile";
  /** Base path để navigate, mặc định "/san-pham" */
  basePath?: string;
  /** Params luôn được giữ lại khi navigate (VD: { q: "hoa hồng" } cho trang tìm kiếm) */
...
export default function FilterSidebar({
  ...
  const filterContent = (
    <div className="space-y-7">
      {/* Chủ đề — chỉ hiện khi có categories */}
      {parentCats.length > 0 && (
        <>
          <div>
  ...
  if (mode === "sidebar") return <>{filterContent}</>;
  if (mode === "mobile") return <>{mobileDrawer}</>;
